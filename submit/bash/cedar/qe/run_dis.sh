...
run_dis(){
    cat >&3 <<EOM
##xx## For DIs:
mpirun -np ${nmpi} \$A/pw.x < ${i}.pawscf.in > ${i}.pawscf.out
mpirun -np ${nmpi} \$A/pp.x < ${i}.rhoae.in > ${i}.rhoae.out
mpirun -np ${nmpi} \$A/pw.x < ${i}.scf.in > ${i}.scf.out
mpirun -np ${nmpi} \$A/pp.x < ${i}.rho.in > ${i}.rho.out
mpirun -np ${nmpi} \$A/open_grid.x < ${i}.opengrid.in > ${i}.opengrid.out
