...
run_dis(){
    cat >&3 <<EOM
##xx## For DIs:
srun -n \$SLURM_NTASKS --mpi=pmi2  \$A/pw.x < ${i}.pawscf.in > ${i}.pawscf.out
srun -n \$SLURM_NTASKS --mpi=pmi2  \$A/pp.x < ${i}.rhoae.in > ${i}.rhoae.out
srun -n \$SLURM_NTASKS --mpi=pmi2  \$A/pw.x < ${i}.scf.in > ${i}.scf.out
srun -n \$SLURM_NTASKS --mpi=pmi2  \$A/pp.x < ${i}.rho.in > ${i}.rho.out
srun -n \$SLURM_NTASKS --mpi=pmi2  \$A/open_grid.x < ${i}.opengrid.in > ${i}.opengrid.out
